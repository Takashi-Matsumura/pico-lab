...
- CodeMirror 6 ベース（`codemirror`, `@codemirror/lang-python`, `@codemirror/theme-one-dark`）
- Python シンタックスハイライト、行番号、括弧マッチング、検索等（`basicSetup`）
- `Compartment` でダーク/ライトテーマを動的切り替え（`MutationObserver` で `<html>` の `.dark` を監視）
- 外部からの `code` prop 変更（AI コード更新）は `dispatch` で反映、`isExternalUpdate` フラグで循環防止
- 実行中は `EditorState.readOnly` で編集不可
